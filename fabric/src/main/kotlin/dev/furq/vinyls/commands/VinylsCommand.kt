...
import dev.furq.vinyls.Vinyls.Companion.messages
import dev.furq.vinyls.Vinyls.Companion.modID
...
class VinylsCommand(private val mod: Vinyls) {
        ...
        CommandRegistrationCallback.EVENT.register { dispatcher: CommandDispatcher<ServerCommandSource>, _: CommandRegistryAccess?, _: CommandManager.RegistrationEnvironment? ->
            dispatcher.register(
                CommandManager.literal("vinyls")
                    ...
                    .executes { context ->
                        context.source.sendError(Text.literal("${prefix} ${messages.getString("command-unknown")}"))
                        0
                    }
            )
        }
    ...
    private fun handleGiveDisc(source: ServerCommandSource, discName: String, playerName: String? = null) {
        val discsMap = discs.getMap("discs")
            ?: return source.sendError(Text.literal("$prefix ${messages.getString("discs-not-found")}"))
        if (discs.getNestedMap(
                discsMap,
                discName
            ) == null
        ) return source.sendError(Text.literal("${prefix} ${messages.getString("disc-not-found")}"))
        //? if <1.21 {
        val material = Registries.ITEM[Identifier(discs.getString("discs.$discName.material").lowercase())]
        //?} else {
        /*val material = Registries.ITEM[Identifier.ofVanilla(discs.getString("discs.$discName.material").lowercase())]
        *///?}

        val customModelData = discs.getInt("discs.$discName.custom_model_data")
        val displayName = discs.getString("discs.$discName.display_name")
        val lore = discs.getList("discs.$discName.lore") as List<String>
        ...
        val discItem = ItemStack(material).apply {
            ...
            *///?}
            ...
            orCreateNbt.putString("${modID}:music_disc", discName)
            orCreateNbt.putString("${modID}:unique_id", UUID.randomUUID().toString())
            //?} else {
            ...
            val nbt = get(DataComponentTypes.CUSTOM_DATA)?.copyNbt() ?: NbtCompound().apply {
                putString("${modID}:music_disc", discName)
            }
        ...
        if (targetPlayer != null) {
            ...
            targetPlayer.sendMessage(
                Text.literal(
                    "${prefix} ${
                        messages.getString("disc-received")
                            ?.replace("{discName}", discName)
                    }"
                )
            )
        } else {
            source.sendError(Text.literal("${prefix} ${messages.getString("player-not-found")}"))
        }
    }

    private fun suggestDiscs(builder: SuggestionsBuilder): CompletableFuture<Suggestions> {
        val discs = discs.getMap("discs").keys.toList()
